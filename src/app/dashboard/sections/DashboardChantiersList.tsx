  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full overflow-hidden rounded-[28px] border border-white/30 bg-white/20 backdrop-blur-xl shadow-xl">
      ...
      <div className="p-5">
        ...
        <p className="mt-1 flex items-center gap-1.5 text-sm text-white/80 drop-shadow-md"><HardHat size={14} /> {chantier.type || "—"} · {formatLocalisation(chantier.localisation)}</p>
        <p className="mt-2 text-sm text-white/80 drop-shadow-md">💰 Budget : <span className="font-bold text-white">{formatFcfa(chantier.budget || 0)}</span></p>
        ...
        {(chantier.statut === "en_attente" || chantier.statut === "en_attente_rdv") && (
          <div className="mt-4 space-y-2">
            {chantier.plan_choisi && <p className="text-sm text-white/80 drop-shadow-md">Plan : <span className="font-bold text-white break-words">{chantier.plan_choisi}</span></p>}
            {chantier.rdv_date && <p className="text-sm text-white/80 drop-shadow-md">📅 RDV : <span className="font-bold text-white">{formatDateCourte(chantier.rdv_date)}</span></p>}
          </div>
        )}
        {(chantier.statut === "termine" || chantier.statut === "terminé") && <p className="mt-4 text-sm text-white/80 drop-shadow-md">Terminé le : <span className="font-bold text-white">{formatDateCourte(chantier.date_fin)}</span></p>}
        <div className="mt-5 flex flex-col gap-3">
          ...
          {chantier.statut === "en_attente" && onModifier && onSupprimer && (
            <div className="flex gap-3">
              <button onClick={() => onModifier(chantier.id)} className="flex-1 rounded-[16px] bg-white/20 py-3 text-sm font-bold text-white transition active:scale-95 shadow-lg">✏️ Modifier</button>
...
export function DashboardChantiersList({ chantiers, isAuthReady, loading, onModifier, onSupprimer }: { chantiers: any[]; isAuthReady: boolean; loading: boolean; onModifier?: (id: string) => void; onSupprimer?: (id: string, statut: string) => void }) {
  if (!isAuthReady) return <div className="flex items-center justify-center py-12 w-full"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#FF7A00]"></div><span className="ml-3 text-white font-bold">Chargement...</span></div>;
