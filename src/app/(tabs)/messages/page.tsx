...
export default async function MessagesPage() {
  ...
  const pageContent = (
    <main className="ios-scroll min-h-screen pt-4 pb-24 px-2">
      <div className="w-full">
          <div className="mb-7 mx-2">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-200 drop-shadow-md">Messages</p>
            <h1 className="mt-2 text-4xl font-black tracking-[-0.04em] text-white md:text-5xl drop-shadow-lg">
              Communication chantier
            </h1>
            <p className="mt-2 text-blue-100 drop-shadow-md">Tous vos échanges avec l'équipe BÂTIZEN.</p>
          </div>
          ...
          <div className="space-y-3 mx-2">
            {messages.map((message) => (
              <article
                key={message.id}
                className="flex items-start gap-4 rounded-[24px] border border-white/30 bg-white/10 backdrop-blur-xl p-5 shadow-lg transition hover:border-white/50 hover:shadow-xl dark:border-[#1D3557]/50"
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 line-clamp-2 text-sm text-blue-100 drop-shadow-md">{message.preview}</p>
                </div>
              </article>
            ))}
          </div>
