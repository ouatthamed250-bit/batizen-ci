...
export default function RenovationPage() {
  ...
  const calculerMontant = useCallback((): number => {
    let total = surface * 500;
    if (etages > 1) total += (etages - 1) * 10000;
    if (horsAbidjan && !transportGere) total += distanceKm * 500;
    return total;
  ...
  }, [user, lieu, surface, etages, horsAbidjan, distanceKm, transportGere, montantEstime, prixVisite, submitting]);
        ...
        <div className="mx-auto max-w-[430px] space-y-6">
            ...
            <>
              ...
              <PremiumCard>
                <div className="space-y-5">
                  <div>
                    <label className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-blue-200"><MapPin size={14} /> Lieu de rénovation</label>
                    <input type="text" value={lieu} onChange={e => setLieu(e.target.value)} placeholder="Ex: Cocody, Abidjan"
                  ...
                  <div>
                    <label className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-blue-200"><Building2 size={14} /> Nombre d'étages</label>
                    <select value={etages} onChange={e => setEtages(parseInt(e.target.value))} className="h-[54px] w-full rounded-[18px] bg-white/10 border border-white/20 px-4 text-sm font-bold text-white outline-none focus:border-[#FF6B00]/50">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="checkbox" id="horsAbidjan" checked={horsAbidjan} onChange={e => setHorsAbidjan(e.target.checked)} className="w-5 h-5 rounded accent-[#FF6B00]" />
                    <label htmlFor="horsAbidjan" className="text-sm font-bold text-white">Le site est situé hors d'Abidjan</label>
                  </div>
                  {horsAbidjan && (
                    <>
                      <div>
                        <label className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-blue-200"><Route size={14} /> Distance depuis Abidjan (km)</label>
                        <input type="number" min={1} max={1000} value={distanceKm || ""} onChange={e => setDistanceKm(parseInt(e.target.value) || 0)} placeholder="Ex: 150"
                          className="h-[54px] w-full rounded-[18px] bg-white/10 border border-white/20 px-4 text-sm font-bold text-white placeholder-blue-300 outline-none focus:border-[#FF6B00]/50" />
                      </div>
                      <div className="flex items-center gap-3">
                        <input type="checkbox" id="transportGere" checked={transportGere} onChange={e => setTransportGere(e.target.checked)} className="w-5 h-5 rounded accent-[#FF6B00]" />
                        <label htmlFor="transportGere" className="text-sm font-bold text-white"><Truck size={14} className="inline mr-1" /> Je gère moi-même le transport</label>
                      </div>
                    </>
                  )}
                </div>
              </PremiumCard>
              <PremiumButton onClick={() => setStep("devis")} className="w-full" icon={ArrowRight}>Voir mon estimation</PremiumButton>
            </>
          )}
            ...
            <>
              ...
              <PremiumCard intensity="high" className="border-t-[6px] border-t-[#FF6B00] text-center">
                ...
                <div className="mt-4 rounded-[16px] bg-white/10 p-4 text-left text-sm text-blue-100 leading-relaxed">
                  Sur la base des informations fournies, voici une estimation indicative de votre projet de rénovation. Ce montant sera affiné après notre visite technique.
                </div>
                <div className="mt-4 rounded-[16px] bg-[#FF7A00]/10 border border-[#FF7A00]/30 p-4 text-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep("form")} className="flex-1 h-[56px] rounded-[18px] bg-white/10 border border-white/20 font-bold text-white transition hover:bg-white/20">Modifier</button>
                <PremiumButton onClick={handleSoumettre} disabled={submitting} className="flex-[2]" icon={CheckCircle2}>{submitting ? "Envoi..." : "Confirmer la demande"}</PremiumButton>
              </div>
            </>
          )}
          ...
          {step === "confirm" && (
            <div className="flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="mt-3 max-w-[280px] text-sm text-blue-100">Après étude, vous serez contacté pour confirmation au bureau et activation du suivi de votre rénovation.</p>
              <div className="mt-8 w-full space-y-3">
                <PremiumButton onClick={() => router.push("/dashboard")} className="w-full">🏠 Retour au tableau de bord</PremiumButton>
                {requestId && <PremiumButton onClick={() => router.push(`/renovation-en-cours/${requestId}`)} variant="secondary" className="w-full">👁️ Voir ma demande</PremiumButton>}
              </div>
            </div>
          )}
